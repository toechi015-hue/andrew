...
export default function AdminDashboard() {
  ...
  async function handleImageUpload(file: File) {
    ...
    try {
      ...
      const imageUrl = apiUrl(`/api/storage${objectPath}`);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const url = isNew ? apiUrl("/api/admin/menu/items") : apiUrl(`/api/admin/menu/items/${editing.id}`);
  ...
  async function handleSaveSides() {
    ...
    try {
      ...
      const res = await fetch(apiUrl("/api/admin/menu/sides"), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
                        ...
                        <div className="flex items-center gap-4">
                          {editing.imageUrl ? (
                            <div className="relative w-20 h-20 rounded-xl overflow-hidden border border-border/40 shrink-0">
                              <img src={mediaSrc(editing.imageUrl)} alt="Menu item" className="w-full h-full object-cover" />
                              <button
